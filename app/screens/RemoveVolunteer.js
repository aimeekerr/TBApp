...
export default function RemoveVolunteer( {navigation, route} ) {
    ...
    const getEmails = async () => { 
        ...
        try {
            let emailsRes = await fetch('http://13.59.212.26/auth/appdb/clinic', request).then((response) => { return response.json(); }).then((myJson) => { console.log(myJson); return myJson })
            setEmails(emailsRes);
            console.log(emails);
        } catch (error) {
    ...
    const deleteEmail = async () => { 
        ...
        try {
            response = await fetch('http://13.59.212.26/auth/appdb/clinic', request).then((response) => { return response.json(); }).then((myJson) => { console.log(myJson); return myJson })
        } catch (error) {
            console.error("The error is", error);
        } finally {
            // if the user is actually in the database -> navigate to the patient info screen
